"""Issue #26：任務條件微調、恢復、policy/reward 評估與三項 gate 核對。"""
import argparse
...
def train(trainer, output, inputs, metric):
    ...
    atomic_save(output / 'recipe.json', seal(dict(model=asdict(trainer.model.dynamics.config),
        training=asdict(trainer.config), stage_one_source=trainer.stage_one_source,
    ...
    try:
        while trainer.step < trainer.config.updates:
            if time.monotonic() >= deadline:
                raise TimeoutError('第二階段已達 6 小時上限')
        ...
        atomic_save(output / 'run.json', seal(dict(schema='dsp-agent-run/1', status=status,
            ...
            updates=trainer.step, seconds=trainer.elapsed_seconds, stage_one_source=trainer.stage_one_source,
            provenance=trainer.provenance, history=trainer.history)))


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('command', choices=['train', 'evaluate', 'predict', 'score-prediction', 'score'])
    ...
    if args.checkpoint:
        ...
        require(args.command == 'train' and args.stage_one is not None, '需提供 --checkpoint 或 --stage-one')
    ...
    provenance['implementation'] = {str(p): file_info(p) for p in [Path(__file__), *[
        ...
        'dynamics.py', 'dynamics_training.py', 'dynamics_evaluation.py')]]}
    ...
    if args.command == 'evaluate':
        ...
        if args.checkpoint:
            trainer = AgentTrainer.restore(args.checkpoint, index, provenance=provenance)
        else:
