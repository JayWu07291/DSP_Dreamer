"""Prepare five source-bound questions or check human answers into a partial draft."""
import argparse
...
def checked_answers(batch, submission):
    ...
    require(submission['schema'] == 'dsp-guided-annotation-answers/1' and submission['batch_id'] == batch['artifact_id']
    ...
    for key, a in submission['answers'].items():
        ...
        r['items'].append(dict(type='item_number', box=q['box'],
            expected=f"{q['target']}：{a['item']}；完整數字={a['count']}" if a['status'] == 'readable' else '',
            eligible=a['status'] == 'readable', exclusion_reason=None if a['status'] == 'readable' else '原圖指定位置的物品或完整數字無法可靠辨識',
