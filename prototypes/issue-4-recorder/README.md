...
這是 issue「製作同步 DSP 錄製與閉迴路控制原型」的拋棄式量測工具。它要回答一個問題：在目標電腦與目前 DSP 版本上，能否用明確的時間對齊契約，以 640×360、10 Hz 連續寫入 RGB 畫面，同時記錄 DSP 實際讀到的鍵鼠狀態、遊戲 tick、任務事件與注入動作，而且掉幀率低於 1%。
...
- `summary.json`：掉幀原因、有效 Hz、寫入量與注入延遲。
...
- 至少做過一次注入，而且每個注入都在 100 ms 內由 `VFInput.OnUpdate` 觀察到。
