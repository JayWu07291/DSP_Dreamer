...
@pytest.mark.parametrize("game_hash, error", [
    ("ae0ba95f75bd879a62aa4ce253b2ab78eaa4fb3c7c595f5e1fee75ebe0e0ef85", "Missing baseline"),
    ("c43a484f6adf8a9e4b956156047070891b46860d5b5c707ba1377b6a2af25732", "Missing baseline"),
    ("f" * 64, "Unknown game fingerprint"),
])
def test_live_lifecycle_without_baseline_cannot_publish(tmp_path, game_hash, error):
    with Recording.synthetic(tmp_path / "source", FFMPEG) as recording:
        recording.begin_attempt(dict(manifest_id="trial", split_group_id="trial", mecha_seed=1, camera_seed=2, policy_seed=3))
        for ticks in (50, 100):
            request = recording.request(ticks, 1, 1)
    ...
    hashes["Assembly-CSharp"] = game_hash
    ...
    with pytest.raises(InvalidRecording, match=error):
