...
.venv/Scripts/python.exe tools/train-tokenizer.py benchmark --output runs/tokenizer-plan.json
```
...
#24 的 64／96-token 對照與恢復證據保留在 [工程驗證紀錄](issue-24-validation.md)。舊 `verify` 命令已退出目前入口，歷史重現使用當時 commit。正式 v1 固定 64 tokens。

四階段已共用[訓練控制器](training.md)，預算、測速、OOM、定期存檔與恢復都以 `runs/training-budget.json` 為準。既有 A 用量連同失敗紀錄會匯入，不重新給四小時。
...
```powershell
.venv/Scripts/python.exe tools/train-tokenizer.py train --output runs/tokenizer-v1
.venv/Scripts/python.exe tools/train-tokenizer.py train --checkpoint runs/tokenizer-v1/final-100.pt --output runs/tokenizer-resumed
```

檔名中的步數只是示例，使用帳本記錄的實際 checkpoint 路徑。`benchmark` 先以完整路徑實測速度固定更新上限。恢復沿用 checkpoint 配方，不重新解讀 `--updates` 或變更 schedule。每 30 分鐘保存 checkpoint，並用固定名單前八張 validation 圖檢查。訓練結束自動排程候選的完整 200 圖 gate；超出時數則保留 pending。Checkpoint 包含配置、資料與凍結身分、metric 權重身分、模型、optimizer、schedule 所需總步數及目前步數、抽樣歷史、Python／NumPy／PyTorch／CUDA RNG；旁邊的 JSON 保存 checkpoint checksum。
...
.venv/Scripts/python.exe tools/train-tokenizer.py score --checkpoint runs/tokenizer-v1/final-100.pt --metrics runs/reconstruction-v1/metrics.json --judgments runs/reconstruction-v1/judgments.json --output runs/reconstruction-v1/reviewed-gate.json
```
...
CPU 測試的 width 32 與兩步序列只供快速回歸。正式 CLI 固定 width 512，正式 throughput 必須由 `benchmark` 在真實資料與 GPU 上產生；恢復與品質證據由 #31 執行。PyTorch peak allocated／reserved 僅表示本行程，不是整卡取樣或閉迴路 VRAM 驗收。
