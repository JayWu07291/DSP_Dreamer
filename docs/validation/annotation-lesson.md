...
示範答案由助理判讀。使用者已於 2026-09-16 提交兩題回答，並確認能理解這種問答方式。顯示示範的練習不能當作未受提示影響的正式標註；後續依既有協定準備原圖題目、可辨識範圍與排除紀錄，不把練習答案升格為凍結標籤。
...
Standards 與 Spec 獨立複核均無剩餘阻擋。使用者實際提交的回答為圖片「鐵塊、10」，影片「空格→有物品圖示、UI」，兩題皆看過示範。原始提交保存在 `runs/annotation-practice-submissions/20260916T085054363Z.json`，維持 `status=practice`、`training_authorized=false`。使用者其後回覆「有」，確認理解指定位置的問答方式；這只完成操作方式的驗收。

後續工作與人工步驟見 [引導式標註準備](guided-annotation.md)。
