...
第十八批、總整合及兩組共 15 題局部標註已完成，不需繼續錄製。Q05 已確認左、右兩座熔爐均未建造完成；兩組共 8 個 UI 框也已確認。最新 27 段移動提案三項均獲 Jay 確認，連同前 10 段共有 37 段完整候選。累計分類為移動 50、UI 63、建造／物品 64、等待 64，四類均達 50，P085 仍待判定。代理已整理原先僅分類的 13 段移動及 64 段等待的範圍與第 15 步狀態，合併為 77 題待核對，不重新要求分類。若全數通過，完整候選為移動 50、等待 64；UI 63 與建造／物品 64 的範圍及狀態仍待整理。原 400 題、補充搜尋 947 個候選與未處理資料保留，最終固定抽樣未凍結。這輪僅讀既有原圖、使用 hard link，未重開 dataset 或重建語料，見 [既有分類的範圍與狀態補齊](sequence-state-completion-20260924.md)。
