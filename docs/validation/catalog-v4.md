...
Jay 先前填寫的 5 題局部物品／數字已保存，無待補回答。4 張來源原圖在新版資料逐像素 hash 相同，映射保存在 `runs/catalog-v4/annotation-source-mapping.json`。這 4 張未被新的固定重建抽樣選中，因此未移植成新名單的答案；原答案、草稿與來源 ID 均保留。新名單的 15 個關鍵項目及 8 個 UI 框已納入正式 annotations，涵蓋協定要求的四種關鍵項目與五類 UI。協定沒有要求逐張確認 200 張圖的所有細節，工作台的整圖確認數不另作門檻。資料與標註凍結已於 9 月 24 日完成，見 [凍結報告](evaluation-data-freeze-20260924.md)；[UI 覆蓋與影片分類](ui-coverage-and-sequence-classification-20260921.md)保留當時紀錄。
...
第十八批、總整合、15 個關鍵項目及 8 個 UI 框均已完成。Jay 確認最後 127 題的範圍與第 15 步狀態後，完整候選為移動 50、UI 63、建造／物品 64、等待 64，共 241 列；已按既定 seed 與整列 hash 固定抽出四類各 50 段。原 200 張重建名單、train baseline、TrainingIndex 及 split 未變。資料與人工標註凍結完成，目前沒有需要補標或補錄的工作。

未抽中的 41 段完整候選、P085 的不確定回答、原 400 題、補充搜尋 947 個候選及人工原文均保留。這次只核對 24 份小型來源 metadata、讀取三份 validation 動作表；沒有重開全 Dataset、讀 RGB、重建語料或複製媒體。詳見 [資料與人工標註凍結報告](evaluation-data-freeze-20260924.md)。

模型品質 gate 仍待驗證，沒有啟動或授權訓練，也未揭露 offline-test 結果。原 40 個保留試驗引用的輸入設定 hash 與目前設定仍不同，正式試驗前須處理此既有差異。
