...
第十八批、總整合及兩組共 15 題局部標註已完成，不需繼續錄製。Q05 已確認左、右兩座熔爐均未建造完成；兩組共 8 個 UI 框也已確認。最新 77 題範圍與第 15 步狀態已獲 Jay 確認，完整候選為移動 50、等待 64，共 114 列，可滿足兩類各 50 的配額。累計分類為移動 50、UI 63、建造／物品 64、等待 64；P085 仍待判定。剩餘 UI 63 與建造／物品 64 的範圍及末幀描述已由代理預填，合併為 127 題待核對，只問兩個新欄位。原 400 題、補充搜尋 947 個候選與未處理資料保留，最終固定抽樣未凍結。這輪只讀既有原圖、使用 hard link，沒有重開 dataset 或重建語料，見 [UI 與建造／物品的範圍及末幀狀態](ui-interaction-state-completion-20260924.md)。
