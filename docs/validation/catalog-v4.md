...
Jay 填寫的 5 題局部物品／數字已保存，無待補回答。4 張來源原圖在新版資料逐像素 hash 相同，映射保存在 `runs/catalog-v4/annotation-source-mapping.json`。這 4 張未被新的固定重建抽樣選中，因此未移植成新名單的答案；原答案、草稿與來源 ID 均保留。新名單整圖確認仍為 0，不能把舊 5 題或新準備的 10 題當作 200 張重建圖或 200 段預測序列驗收完成。
...
第十八批與總整合已完成，不需繼續錄製。下一步是在 `http://127.0.0.1:8829/` 完成新名單的第一組 10 題指定位置物品／數字，逐題儲存後匯出 JSON 貼回對話。具體步驟見 [正式標註準備](corpus-integration-20260921.md)。
