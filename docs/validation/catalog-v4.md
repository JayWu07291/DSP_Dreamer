...
後續第八批已核對完畢，並依使用者決定維持目前動作空間。目前操作請依 [任務正例缺口診斷](coverage-diagnosis-batch8.md)，先錄第九批一回合。第八批資料與第九批設定紀錄見 [第八批錄製核對](recording-batch8.md)；最新完整總索引與收件方式見 [第五批錄製核對](recording-batch5.md)。

第八批缺口原因已核對：拆解事件存在，但開科技樹時的錄製缺口與按鍵重設切斷了不足 64 步的片段；熔爐接線等部分完成發生於非 active 任務。第九批改為先錄 **一回合** 驗證開局順序，具體操作與停止條件見 [任務正例缺口診斷](coverage-diagnosis-batch8.md)。這項安排取代下方及第八批歷史報告的固定兩回合指示。
...
第八批已完成，第九批沿用已套用設定，但先依 [任務正例缺口診斷](coverage-diagnosis-batch8.md) 錄一回合。維持「電磁學完成 → 鐵礦採礦機 → 銅礦採礦機 → 冶金供料」的採礦優先安排，中後段依自然遊玩交錯進行。B／X 可正常使用，先前錄製保留。
