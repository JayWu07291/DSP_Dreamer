...
Jay 先前填寫的 5 題局部物品／數字已保存，無待補回答。4 張來源原圖在新版資料逐像素 hash 相同，映射保存在 `runs/catalog-v4/annotation-source-mapping.json`。這 4 張未被新的固定重建抽樣選中，因此未移植成新名單的答案；原答案、草稿與來源 ID 均保留。新名單的第一組 10 題也已收件並匯入，無漏答，見 [標註收件與下一組狀態題](guided-annotations-20260921.md)。整圖確認仍為 0，局部回答不能當作 200 張重建圖或 200 段預測序列驗收完成。
...
第十八批、總整合及兩組共 15 題收件已完成，涵蓋 7 張來源原圖，不需繼續錄製。Q05 已由使用者補充確認，左、右兩座熔爐均未建造完成。首組 4 個 UI 框也已確認，涵蓋 2 張原圖的物品清單與合成器。下一步是在 `http://127.0.0.1:8833/` 查看科技樹、熔爐操作與研究站的 4 個框，直接回到對話確認或指出需修正之處。具體步驟見 [首組 UI 確認及下一步](ui-regions-confirmed-20260921.md)。
