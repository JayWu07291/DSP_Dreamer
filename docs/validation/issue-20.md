...
目前已完成 v3／20 維模型動作注入、校正與安全釋放的驗收。最新六條要求對照、互斥測試與交接限制見[結案驗收](issue-20-closeout.md)。以下保留逐次開發歷史；「待驗證」描述屬當時狀態。依使用者要求，實機均由使用者操作遊戲，不使用 Computer Use。

2026-09-12 更新：本輪安排的控制與故障實機驗證已完成，包含新增持續按鍵失焦、最後觀察讀回失敗、Recorder 元件 OnDisable／OnDestroy，以及銷毀後重啟完整探針。最新結果見[專用診斷驗收紀錄](issue-20-diagnostics.md)；下方「仍待」描述保留各階段當時狀態，不代表最新驗收進度。尚不宣稱 policy runner 或 #31 的代理成果 gate。

持續按鍵失焦、final-only readback failure 與插件元件停用／銷毀入口已完成新版實機驗收，見[專用診斷流程與結果](issue-20-diagnostics.md)。已有核准校正的部署不必因文件更新而重新校正。
